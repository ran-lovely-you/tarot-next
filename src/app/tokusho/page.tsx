export default function TokushoPage() {
  const rows = [
    ...
    ['メールアドレス', 'ran-miyoshi@outlook.jp'],
    ['サービス名', 'タロット占い 天星鑑定'],
    ...
    ['返品・キャンセル', 'デジタルコンテンツの性質上、購入後の返品・返金は原則お受けしておりません。\n月額サブスクは次回更新日前日までにキャンセル手続きで翌月以降の請求を停止できます。'],
  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      <nav style={{ display: 'flex', alignItems: 'center', padding: '14px 28px', borderBottom: '1px solid #4a2e7a' }}>
        <a href="/" style={{ fontFamily: "'Cinzel Decorative',serif", color: '#d4af37', fontSize: 15, letterSpacing: '0.08em', textDecoration: 'none' }}>✦ タロット占い 天星鑑定</a>
      </nav>
          ...
          <tbody>
            {rows.map(([th, td]) => (
              <tr key={th}>
                ...
                <td style={{ padding: '12px 16px', border: '1px solid #4a2e7a', color: '#e8e4f8', lineHeight: 1.9, whiteSpace: 'pre-line' }}>{td}</td>
              </tr>
            ))}
          </tbody>
        </table>
